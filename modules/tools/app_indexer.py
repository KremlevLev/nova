...
def get_visible_window_titles() -> list[str]:
    ...
    def callback(hwnd, _lparam):
        try:
            ...
            logger.debug(
                "Не удалось прочитать заголовок окна.",
                exc_info=True,
            )
...
class WindowsAppIndexer:
    ...
    def index_all_apps(self) -> None:
        ...
        for base_path in self._get_start_menu_paths():
            if not base_path.exists():
                logger.debug(
                    "Каталог меню Пуск отсутствует: %s",
                    base_path,
                )
                continue
            ...
            try:
                for shortcut_path in base_path.rglob("*"):
                    ...
                    # Первый найденный ярлык сохраняем. Это делает
                    # индекс стабильнее при одинаковых именах.
                    indexed_apps.setdefault(
                        app_name,
                        str(shortcut_path),
                    )
        ...
        logger.info(
            "Индексация ПО завершена. Найдено ярлыков: %s",
            len(self.apps_cache),
        )
    ...
    def _resolve_alias(query: str) -> str:
        ...
        alias_target = RUSSIAN_ALIASES.get(
            normalized_query,
            normalized_query,
        )
    ...
    def _find_system_fallback(
        ...
        executable = (
            SYSTEM_FALLBACKS.get(query_clean)
            or SYSTEM_FALLBACKS.get(alias_target)
        )
    ...
    def find_app(
        ...
        system_match = self._find_system_fallback(
            query_clean
        )
        ...
        # Старое значение 0.55 могло открыть совершенно не то ПО.
    ...
    def _expected_window_terms(
        ...
        executable_stem = normalize_app_name(
            Path(match.path).stem
        )

        if executable_stem:
            terms.add(executable_stem)

        # Слишком короткие термы создают много ложных совпадений.
        return {
            term
            for term in terms
            if len(term) >= 3
    ...
    def launch_by_name(
        ...
        if match is None:
            return (
                False,
                f"Приложение '{app_name}' не найдено.",
            )

        if (
            match.match_type == "fuzzy"
            and match.score < 0.82
        ):
            return (
                False,
                (
                    "Найдено похожее приложение "
                    f"'{match.matched_name}', но уверенность "
                    f"слишком низкая: {match.score:.0%}."
                ),
            )

        expected_terms = self._expected_window_terms(
            app_name,
            match,
        )
        ...
        # Если приложение уже открыто, os.startfile может просто
        # передать ему повторную команду запуска. Это допустимо.
        existing_window = self._find_matching_window(
            before_titles,
            expected_terms,
        )
        ...
        try:
            ...
            logger.exception(
                "Windows не смогла запустить %s.",
                match.path,
            )
            return (
                False,
                f"Ошибка запуска '{app_name}': {exc}",
            )
            ...
            return (
                False,
                f"Непредвиденная ошибка запуска: {exc}",
            )
            ...
            if matching_window:
                if existing_window:
                    return (
                        True,
                        (
                            f"Приложение '{match.matched_name}' "
                            "уже было открыто. Запрос передан "
        ...
        if last_new_titles:
            logger.info(
                "После запуска появились окна с неожиданными "
                "заголовками: %s",
                sorted(last_new_titles),
            )

        return (
            True,
            (
                f"Запрос на запуск '{match.matched_name}' "
                "передан Windows, но появление окна "
                "не подтверждено."
            ),
        )
